export interface QuestTemplate {
  title: string;
  description: string;
  path: 'Scholar' | 'Warrior' | 'Sage' | 'Creator';
...
export function selectQuestsForUser(pathScores: { scholar: number, warrior: number, sage: number, creator: number }): QuestTemplate[] {
  const getDifficulty = (score: number): 'Easy' | 'Medium' | 'Hard' | 'Elite' => {
  ...
  const resultQuests: QuestTemplate[] = [];
  const paths: ('Scholar' | 'Warrior' | 'Sage' | 'Creator')[] = ['Scholar', 'Warrior', 'Sage', 'Creator'];

  for (const path of paths) {
    let score = 50;
    if (path === 'Scholar') score = pathScores.scholar;
    else if (path === 'Warrior') score = pathScores.warrior;
    else if (path === 'Sage') score = pathScores.sage;
    else if (path === 'Creator') score = pathScores.creator;

    const diff = getDifficulty(score);
    let candidates = QUEST_TEMPLATES.filter(t => t.path === path && t.difficulty === diff);
    if (candidates.length === 0) {
      candidates = QUEST_TEMPLATES.filter(t => t.path === path);
    }
